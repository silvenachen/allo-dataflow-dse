...
int main(int argc, char** argv) {
    ...
    cl::Kernel krnl_kernel_gemm_transpose;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_kernel_gemm_transpose = cl::Kernel(program, "kernel_gemm_transpose", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_kernel_gemm_transpose.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_kernel_gemm_transpose.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_kernel_gemm_transpose.setArg(2, buffer_out0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_kernel_gemm_transpose, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "kernel_gemm_transpose "
