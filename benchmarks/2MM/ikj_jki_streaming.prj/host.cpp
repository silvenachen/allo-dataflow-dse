...
int main(int argc, char** argv) {
    ...
    cl::Kernel krnl_two_mm_stream_ikj_jki;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_two_mm_stream_ikj_jki = cl::Kernel(program, "two_mm_stream_ikj_jki", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_two_mm_stream_ikj_jki.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_two_mm_stream_ikj_jki.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_two_mm_stream_ikj_jki.setArg(2, buffer_in2));
    OCL_CHECK(err, err = krnl_two_mm_stream_ikj_jki.setArg(3, buffer_in3));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_two_mm_stream_ikj_jki, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "two_mm_stream_ikj_jki "
