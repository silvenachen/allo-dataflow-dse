...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_kernel_2mm = cl::Kernel(program, "kernel_2mm", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_kernel_2mm.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_kernel_2mm.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_kernel_2mm.setArg(2, buffer_in2));
    OCL_CHECK(err, err = krnl_kernel_2mm.setArg(3, buffer_out0));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_kernel_2mm, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "kernel_2mm "
    ...
    for (unsigned i = 0; i < source_out0.size(); i++) {
        ofile << source_out0[i] << std::endl;
    }
    ofile.close();
