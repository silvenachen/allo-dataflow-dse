...
int main(int argc, char** argv) {
    ...
    cl::Kernel krnl_two_mm_stream_jki_jki;
    ...
    float in_data_3[4096];
    for (unsigned i = 0; i < 4096; i++) {
      ifile3 >> in_data_3[i];
    }
    size_t size_bytes_in3 = sizeof(float) * 4096;
    std::vector<float, aligned_allocator<float> > source_in3(in_data_3, in_data_3 + 4096);
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_two_mm_stream_jki_jki = cl::Kernel(program, "two_mm_stream_jki_jki", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, cl::Buffer buffer_in3(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, size_bytes_in3, source_in3.data(), &err));

    OCL_CHECK(err, err = krnl_two_mm_stream_jki_jki.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_two_mm_stream_jki_jki.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_two_mm_stream_jki_jki.setArg(2, buffer_in2));
    OCL_CHECK(err, err = krnl_two_mm_stream_jki_jki.setArg(3, buffer_in3));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_two_mm_stream_jki_jki, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "two_mm_stream_jki_jki "
