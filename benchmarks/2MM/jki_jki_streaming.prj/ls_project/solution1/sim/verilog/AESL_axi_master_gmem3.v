...
//------------------------Parameter----------------------
`define TV_IN_gmem3 "../tv/cdatafile/c.two_mm_stream_jki_jki.autotvin_gmem3.dat"
`define TV_OUT_gmem3 "../tv/rtldatafile/rtl.two_mm_stream_jki_jki.autotvout_gmem3.dat"
 parameter gmem3_ADDR_BITWIDTH = 32'd 64;
 parameter gmem3_AWUSER_BITWIDTH = 32'd 1;
 parameter gmem3_DATA_BITWIDTH = 32'd 512;
...
parameter gmem3_C_DATA_BITWIDTH = 32'd 512;
parameter gmem3_mem_depth = 32'd 256;
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("/export/hdd/scratch/sli3079/allo_ls/allo_ls_dse/benchmarks/2MM/jki_jki_streaming.prj/ls_project/solution1/sim/verilog/AESL_axi_master_gmem3.v: Write request address %d exceed AXI master gmem3 array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem3_mem_depth) begin
                $display ("/export/hdd/scratch/sli3079/allo_ls/allo_ls_dse/benchmarks/2MM/jki_jki_streaming.prj/ls_project/solution1/sim/verilog/AESL_axi_master_gmem3.v: Read request address %d exceed AXI master gmem3 array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem3_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  reg [1047 : 0] token; 
  reg [1047 : 0] token_tmp; 
  //reg [gmem3_C_DATA_BITWIDTH - 1 : 0] token_tmp; 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  case(mem_page)
                      ...
                      3 : gmem3_mem_3[i/factor] = mem_tmp;
  ...
  while(1) begin 
      ...
      for(i = 0; i < (gmem3_mem_depth - gmem3_mem_depth % factor); i = i + 1) begin 
          if (factor == 4) begin
              case(mem_page)
                  ...
                  3 : tmp_data = gmem3_mem_3[i/factor];
      ...
      if (factor == 4) begin
          case(mem_page)
              ...
              3 : tmp_data = gmem3_mem_3[gmem3_mem_depth/factor];
              default: $display("The page_num of write file is not valid!");
          endcase
      ...
      if (factor == 2) begin
          if ((gmem3_mem_depth - 1) % factor == 0) begin
              case(mem_page)
                  0 : $fdisplay(fp,"0x%x",gmem3_mem_0[gmem3_mem_depth/factor][15:0]);
                  1 : $fdisplay(fp,"0x%x",gmem3_mem_1[gmem3_mem_depth/factor][15:0]);
                  2 : $fdisplay(fp,"0x%x",gmem3_mem_2[gmem3_mem_depth/factor][15:0]);
                  3 : $fdisplay(fp,"0x%x",gmem3_mem_3[gmem3_mem_depth/factor][15:0]);
                  default: $display("The page_num of write file is not valid!");
              endcase
          end
      end
