...
int main(int argc, char** argv) {
    ...
    cl::Kernel krnl_gemm_relu_row_wise;
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_gemm_relu_row_wise = cl::Kernel(program, "gemm_relu_row_wise", &err));
            valid_device = true;
    ...
    OCL_CHECK(err, err = krnl_gemm_relu_row_wise.setArg(0, buffer_in0));
    OCL_CHECK(err, err = krnl_gemm_relu_row_wise.setArg(1, buffer_in1));
    OCL_CHECK(err, err = krnl_gemm_relu_row_wise.setArg(2, buffer_in2));
    ...
    // Launch the Kernel
    OCL_CHECK(err, err = q.enqueueTask(krnl_gemm_relu_row_wise, nullptr, &event));
    ...
    std::cout << "| " << std::left << std::setw(24) << "gemm_relu_row_wise "
