...
protected:
    ...
    // buffer instances
    top_buf1_memcore* buffer[BufferCount];
...
public:
    ~top_buf1();
    SC_CTOR(top_buf1) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "top_buf1_memcore_U_";
            name += szTmp;

            buffer[i] = new top_buf1_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "top_buf1.trace");
            // ports
