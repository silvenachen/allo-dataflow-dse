...
#ifndef _top_v44_buf_data_H_
#define _top_v44_buf_data_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "top_v44_buf_data_memcore.h"

SC_MODULE(top_v44_buf_data) {
    ...
    static const unsigned int IndexWidth   = 2;
...
protected:
    ...
    // buffer instances
    top_v44_buf_data_memcore* buffer[BufferCount];
...
public:
    ~top_v44_buf_data();
    SC_CTOR(top_v44_buf_data) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "top_v44_buf_data_memcore_U_";
            name += szTmp;

            buffer[i] = new top_v44_buf_data_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "top_v44_buf_data.trace");
            // ports
