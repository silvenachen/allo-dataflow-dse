...
protected:
    ...
    top_buf2_memcore* top_buf2_memcore_U;
...
public:
    ~top_buf2();
    SC_CTOR(top_buf2) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        top_buf2_memcore_U = new top_buf2_memcore("top_buf2_memcore_U");
        top_buf2_memcore_U->clk      ( clk );
        top_buf2_memcore_U->reset    ( reset );
        top_buf2_memcore_U->ce0      ( i_ce0 );
        top_buf2_memcore_U->we0      ( i_we0 );
        top_buf2_memcore_U->address0 ( memcore_iaddr );
        top_buf2_memcore_U->d0       ( i_d0 );
        top_buf2_memcore_U->q0       ( i_q0 );

        top_buf2_memcore_U->ce1      ( t_ce0 );
        top_buf2_memcore_U->we1      ( t_we0 );
        top_buf2_memcore_U->address1 ( memcore_taddr );
        top_buf2_memcore_U->d1       ( t_d0 );
        top_buf2_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "top_buf2.trace");
            // ports
