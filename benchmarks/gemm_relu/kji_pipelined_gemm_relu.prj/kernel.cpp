...
void gemm_stage(
  ...
  l_S_i_j_k_0_k: for (int k = 0; k < 64; k++) {	// L3
    l_j: for (int j = 0; j < 64; j++) {	// L4
      l_i: for (int i = 0; i < 64; i++) {	// L5
...
void kernel_gemm_relu(
  ...
  #pragma HLS interface m_axi port=v33 offset=slave bundle=gmem0 depth=4096 depth=4096
  #pragma HLS interface m_axi port=v34 offset=slave bundle=gmem1 depth=4096 depth=4096
  #pragma HLS interface m_axi port=v35 offset=slave bundle=gmem2 depth=4096 depth=4096
  float buf0[64][64];	//
  load_buf0(v33, buf0);	//
  float buf1[64][64];	//
  load_buf1(v34, buf1);	//
